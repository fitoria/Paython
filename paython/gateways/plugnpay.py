...
class PlugnPay(PostGateway):
    ...
    def __init__(
        self, username="pnpdemo", password="", email="", dontsndmail=True, debug=True
    ):
    ...
    def auth(self, amount, credit_card=None, billing_info=None, shipping_info=None):
        ...
        if not credit_card:
            debug_string = (
                "paython.gateways.plugnpay.auth()  -- No CreditCard object present. You passed in %s "
                % (credit_card)
            )
    ...
    def capture(self, amount, credit_card=None, billing_info=None, shipping_info=None):
        ...
        if not credit_card:
            debug_string = (
                "paython.gateways.plugnpay.capture()  -- No CreditCard object present. You passed in %s "
                % (credit_card)
            )
            logger.debug(debug_string)
    ...
    def request(self):
        ...
        debug_string = (
            " paython.gateways.plugnpay.request()  -- Request completed in %ss "
            % response_time
        )
        logger.debug(debug_string.center(80, "="))
    ...
    def parse(self, raw_response, response_time):
        ...
        for field in raw_response:
            t = field.split("=")
            response[urllib.parse.unquote(t[0])] = (
                urllib.parse.unquote(t[1]).strip("|").strip()
            )

        # map AVS code to string based on `card-type`
        if "avs-code" in response:
            if (
                response.get("card-type") in self.AVS_RESPONSE_KEYS
                and response["avs-code"]
                in self.AVS_RESPONSE_KEYS[response["card-type"]]
            ):
                response["avs-code-msg"] = self.AVS_RESPONSE_KEYS[
                    response["card-type"]
                ][response["avs-code"]]
            elif (
                response["avs-code"] in self.AVS_RESPONSE_KEYS["VISA"]
            ):  # default to VISA AVS description
                response["avs-code-msg"] = self.AVS_RESPONSE_KEYS["VISA"][
                    response["avs-code"]
                ]
        ...
        if (
            "resp-code" in response
            and response["resp-code"] in self.STATUS_RESPONSE_KEYS
        ):
        ...
        return super(PlugnPay, self).standardize(
            response, self.RESPONSE_KEYS, response_time, approved
        )
