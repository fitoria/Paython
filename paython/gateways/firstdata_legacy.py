...
class FirstDataLegacy(XMLGateway):
    ...
    API_URI = {"live": "https://secure.linkpt.net/LSGSXML"}
    ...
    def __init__(
        self,
        username="Test123",
        key_file="../keys/yourkey.pem",
        cert_file="../keys/yourkey.pem",
        debug=False,
        test=False,
    ):
        """
        ...
        super(FirstDataLegacy, self).__init__(
            url,
            translations=self.REQUEST_FIELDS,
            debug=debug,
            special_params=ssl_config,
        )
    ...
    def charge_setup(self):
        ...
        if self.cvv_present:
            super(FirstDataLegacy, self).set(
                "order/creditcard/cvmindicator", "provided"
            )
    ...
    def auth(self, amount, credit_card=None, billing_info=None, shipping_info=None):
        ...
        if matches:
            super(FirstDataLegacy, self).set(
                "order/billing/addrnum", matches.group()
            )  # hardcoded because of uniqueness to gateway
        else:
            raise DataValidationError(
        ...
        if not credit_card:
            debug_string = (
                "paython.gateways.firstdata_legacy.auth()  -- No CreditCard object present. You passed in %s "
                % (credit_card)
            )
    ...
    def credit(self, amount, trans_id, credit_card):
        ...
        super(FirstDataLegacy, self).set(self.REQUEST_FIELDS["trans_id"], trans_id)
        super(FirstDataLegacy, self).set(
            self.REQUEST_FIELDS["number"], credit_card.number
        )
    ...
    def request(self):
        ...
        debug_string = (
            " paython.gateways.firstdata_legacy.request() -- Attempting request to: "
        )
        logger.debug(debug_string.center(80, "="))
        logger.debug(
            "\n %s with params: %s"
            % (self.API_URI["live"], super(FirstDataLegacy, self).request_xml())
        )
        ...
        debug_string = (
            " paython.gateways.firstdata_legacy.request()  -- Request completed in %ss "
            % response_time
        )
        logger.debug(debug_string.center(80, "="))
    ...
    def parse(self, response, response_time):
        ...
        debug_string = (
            " paython.gateways.firstdata_legacy.parse() -- Response as dict: "
        )
        logger.debug(debug_string.center(80, "="))
        logger.debug("\n%s" % response)

        return super(FirstDataLegacy, self).standardize(
