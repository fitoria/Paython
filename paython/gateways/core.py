...
class Gateway(object):
    ...
    def set_shipping_info(
        ...
        optionals = [
            ...
            "ship_email",
            "ship_country",
        ]  # using list of strings for reasons spec'd below

        # in line comments on this one
        for optional_var in optionals:
            exec(
                "%s = %s" % (optional_var, optional_var)
            )  # re-assign each option param to itself
            if eval(optional_var):  # see if it was passed into the method
                if (
                    optional_var not in self.REQUEST_FIELDS
                ):  # make sure we have a translation in the request fields dictionary
                    # & keep the string so we have a meaningful exception
                    raise MissingTranslationError(
                        'Gateway doesn\'t support the "%s" field for shipping'
                        % optional_var
                    )
    ...
    def standardize(self, spec_response, field_mapping, response_time, approved):
        ...
        if isinstance(spec_response, list):  # list settings
            ...
            for item in spec_response:
                iteration_key = str(
                    i
                )  # stringifying because the field_mapping keys are strings
