...
class Stripe(object):
    ...
    def auth(self, amount, credit_card=None, billing_info=None, shipping_info=None):
        ...
        raise NotImplementedError(
            "Stripe does not support auth or settlement. Try capture()."
        )
    ...
    def void(self, trans_id):
        ...
        """
        raise NotImplementedError(
            "Stripe does not support transaction voiding. Try credit()."
        )
    ...
    def parse(self, response, response_time):
        ...
        # trans type ;)
        new_response["trans_type"] = (
            "credit" if response.get("amount_refunded") > 0 else "capture"
        )
