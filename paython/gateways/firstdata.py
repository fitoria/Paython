...
class FirstData(PostGateway):
    ...
    API_URI = {False: GATEWAY_LIVE, True: GATEWAY_TEST}
    RESPONSE_FIELDS = {}
    TRANSACTION_TYPES = {
        "purchase": "00",
    ...
    def use_credit_card(self, credit_card):
        ...
        """
        if hasattr(
            credit_card, "_exp_yr_style"
        ):  # here for gateways that like 2 digit expiration years
    ...
    def auth(self, amount, cc_obj):
        ...
        self.set("password", self.password)
        self.set(
            self.REQUEST_FIELDS["trans_type"],
            self.TRANSACTION_TYPES["pre_authorization"],
        )
        self.set(self.REQUEST_FIELDS["amount"], amount)
        self.use_credit_card(cc_obj)
        r = self.request()
    ...
    def auth_completion(self, amount, cc_obj, auth_num):
        ...
        self.set(
            self.REQUEST_FIELDS["trans_type"],
            self.TRANSACTION_TYPES["pre_authorization_completion"],
        )
    ...
    def tagged_refund(self, amount, transaction_tag, auth_num):
        ...
        self.set("password", self.password)
        self.set(
            self.REQUEST_FIELDS["trans_type"], self.TRANSACTION_TYPES["tagged_refund"]
        )
        self.set(self.REQUEST_FIELDS["amount"], amount)
    ...
    def tagged_void(self, amount, transaction_tag, auth_num):
        ...
        self.set("password", self.password)
        self.set(
            self.REQUEST_FIELDS["trans_type"], self.TRANSACTION_TYPES["tagged_void"]
        )
        self.set(self.REQUEST_FIELDS["amount"], amount)
        self.set("transaction_tag", transaction_tag)
        self.set("authorization_num", auth_num)
    ...
    def tagged_pre_authorization_completion(self, amount, transaction_tag, auth_num):
        ...
        self.set(
            self.REQUEST_FIELDS["trans_type"],
            self.TRANSACTION_TYPES["tagged_pre_authorization_completion"],
        )
        self.set(self.REQUEST_FIELDS["amount"], amount)
        self.set("transaction_tag", transaction_tag)
        self.set("authorization_num", auth_num)
    ...
    def request(self, retry_on_bmc=1):
        ...
        r = requests.post(
            ("https://" + self.url + "/transaction/v14"),
            timeout=20,
    ...
    def parse(self, response):
        ...
        if (
            type(self._retry_on_bmc) is int
            and 0 < self._retry_on_bmc < 4
            and response == "Unauthorized Request. Bad or missing credentials."
        ):
            ...
            try:
                ...
                try:
                    ...
                    error = {
                        ...
                        "amount": self.REQUEST_DICT.get("amount", 0),
                        ...
                        "payer_id": None,
                        "transaction_type": self.REQUEST_DICT.get("transaction_type"),
                        "cc_verification_str2": None,
                        "ecommerce_flag": None,
                        "reference_no": self.REQUEST_DICT.get("reference_no"),
                        "cavv": None,
                        "previous_balance": None,
                        "error_description": None,
                        "tax2_number": None,
                        "exact_resp_code": None,
                        "secure_auth_required": None,
