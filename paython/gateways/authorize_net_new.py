...
class AuthorizeNetNew(JsonGateway):
    ...
    def __init__(
        ...
        super(AuthorizeNetNew, self).__init__(
            translations=self.REQUEST_FIELDS, debug=debug
        )
        super(AuthorizeNetNew, self).set(
            "merchantAuthentication", {"name": username, "transactionKey": password}
        )
        ...
        if test:
            if test != self.LIVE_TEST:
                ...
                test_string = "live"
            debug_string = (
                " paython.gateways.authorize_net.__init__() -- You're in %s test mode (& debug, obviously) "
                % test_string
            )
    ...
    def charge_setup(
        self,
        api_call_name="createTransactionRequest",
        method_call_name="transactionRequest",
    ):
        """
        ...
        debug_string = (
            " paython.gateways.authorize_net.charge_setup() Just set up for a charge "
        )
        logger.debug(debug_string.center(80, "="))
    ...
    def set_billing_info(self, **kwargs):
        ...
        logger.debug(
            "paython.gateways.authorize_net.set_billing_info() -- Just set up billing info "
        )
    ...
    def set_shipping_info(self, **kwargs):
        ...
        logger.debug(
            "paython.gateways.authorize_net.set_shipping_info() -- Just set up billing info "
        )
    ...
    def auth(
        ...
        self.REQUEST_DICT["transactionRequest"][
            "transactionType"
        ] = "authOnlyTransaction"
    ...
    def create_subscription(
        ...
        )  # considering turning this into a decorator?
    ...
    def cancel_subscription(self, subscription_id):
        """Creates a subscription (recurring billing)"""
        self.charge_setup(
            "ARBCancelSubscriptionRequest", "subscriptionId"
        )  # considering turning this into a decorator?
    ...
    def request(self):
        ...
        if self.test == self.LIVE_TEST or not self.test:
            ...
            url = self.API_URI[
                "test"
            ]  # here just in case we want to granularly change endpoint

        debug_string = (
            " paython.gateways.authorize_net.request() -- Attempting request to: "
        )
        logger.debug(debug_string.center(80, "="))
        debug_string = "%s with params: %s" % (
            url,
            super(AuthorizeNetNew, self).params(),
        )
        logger.debug(debug_string)
        ...
        debug_string = (
            " paython.gateways.authorize_net.request()  -- Request completed in %ss "
            % response_time
        )
        logger.debug(debug_string.center(80, "="))
