...
class AuthorizeNet(PostGateway):
    ...
    def __init__(
        ...
        super(AuthorizeNet, self).__init__(
    ...
    def auth(
        ...
        if invoice_num is not None:
            super(AuthorizeNet, self).set(
                self.REQUEST_FIELDS["invoice_num"], invoice_num
            )
        ...
        if is_partial:
            ...
            super(AuthorizeNet, self).set(
                self.REQUEST_FIELDS["split_tender_id"], split_id
            )
    ...
    def settle(self, amount, trans_id, split_id=None):
        ...
        # setting transaction data
        super(AuthorizeNet, self).set(
            self.REQUEST_FIELDS["trans_type"], "PRIOR_AUTH_CAPTURE"
        )
        super(AuthorizeNet, self).set(self.REQUEST_FIELDS["amount"], amount)
    ...
    def capture(self, amount, credit_card=None, billing_info=None, shipping_info=None):
        ...
        if not credit_card:
            debug_string = (
                "paython.gateways.authorize_net.capture()  -- No CreditCard object present. You passed in %s "
                % (credit_card)
            )
            logger.debug(debug_string)
    ...
    def void(self, trans_id, split_id=None):
        ...
        if (
            split_id
        ):  # voids an entire split (alternatively, a trans_id just kills that particular txn)
            super(AuthorizeNet, self).set(
                self.REQUEST_FIELDS["split_tender_id"], split_id
            )
            super(AuthorizeNet, self).unset(self.REQUEST_FIELDS["trans_id"])
    ...
    def parse(self, response, response_time):
        ...
        return super(AuthorizeNet, self).standardize(
